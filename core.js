...
(function (global) {
  ...
  function mountViewer(rootEl, rawText, opts) {
    ...
    // ---- feature 3: Schema / TypeScript export + Schema validation (⋯ menu export group) ----
    const wrapEl = $(".jk-wrap");
    ...
    function codePanel(title, code, uncertainties, filename) {
      const p = JK.panel.open(wrapEl, {
        ...
        actions: [
          ...
          { label: "Download", onClick: () => { download(filename, code); bar.setFlash("Downloaded ✓"); } },
        ],
      });
    ...
    function exportSchema() {
      const { schema, uncertainties } = JK.schema.infer(displayValue);
      codePanel("JSON Schema", JSONBig.stringify(schema, 2), uncertainties, "schema.json");
    }
    function exportTS() {
      const { code, uncertainties } = JK.schema.toTypeScript(displayValue, {});
    ...
    function openValidate() {
      ...
      const p = JK.panel.open(wrapEl, {
        ...
        onClose: () => { if (tree) tree.clearInvalid(); },
      });
      p.body.innerHTML =
    ...
    function runValidate(body) {
      ...
      let schema;
      // JSONBig, not JSON.parse: a schema's own big-integer bounds must survive parsing. normalize
      // tolerates a pasted XSSI-guarded / JSONP-wrapped schema the same as the document does.
      try { schema = JSONBig.parse(normalize(ta.value)); }
      catch (e) {
        // In-place error; the document view is untouched (AC-208). createElement + textContent
        // rather than innerHTML: the message quotes the pasted schema, so there's no place to
        // inject into (L-014), and no need to esc.
        out.innerHTML = "";
      ...
      const { ok, errors } = JK.schema.validate(schema, displayValue);
      renderTree();
      ...
      // createElement, not an innerHTML string: these are clickable, and building them as real
      // elements is what lets the click land (and be tested) rather than a querySelectorAll over a
      // string. The keyword/path/message all embed the schema and doc, so they go through
      // textContent — no escaping to forget.
      errors.forEach((e) => {
        ...
        btn.addEventListener("click", () => { if (e.apath !== undefined) jumpToPath(e.apath); });
        out.appendChild(btn);
      });
    }
