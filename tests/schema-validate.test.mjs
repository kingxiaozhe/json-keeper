...
// 审查 Finding 1 的根治：不只测"黑名单里的会报"（那只覆盖我自己列过的），而是拿一份**手抄的
// 2020-12 断言词表**逐个核对 —— 每个我不支持的断言型关键字都必须被明确提示，否则就是静默放过
// = 假"通过"。这道 cross-check 就是被删掉的那层交叉验证，L-009 的正解。
test("断言词表 cross-check：每个未实现的 2020-12 断言型关键字都必须被提示，不许静默放过", () => {
  // 手抄的 2020-12 断言/适用型关键字（会影响校验结论的）。已支持的不列。
  const UNSUPPORTED_ASSERTIONS = [
    "allOf", "anyOf", "oneOf", "not", "if", "then", "else",
    "patternProperties", "propertyNames", "pattern", "format",
    "uniqueItems", "multipleOf", "contains", "minContains", "maxContains",
    "prefixItems", "minProperties", "maxProperties",
    "dependentSchemas", "dependentRequired", "unevaluatedProperties", "unevaluatedItems",
  ];
  for (const kw of UNSUPPORTED_ASSERTIONS) {
    const r = V({ [kw]: kw === "not" ? {} : (kw.startsWith("min") || kw.startsWith("max") || kw === "multipleOf" ? 1 : []) }, { anything: 1 });
    assert.ok(r.errors.some((e) => e.keyword === kw),
      `断言型关键字 "${kw}" 未支持却被静默忽略了 —— 这是假"通过"，比不支持更危险`);
  }
});

test("审查 Finding 1：prefixItems / minProperties / maxProperties 曾静默放过，现在报", () => {
  assert.equal(V({ prefixItems: [{ type: "string" }] }, [1, 2]).ok, false, "prefixItems 不该被忽略");
  assert.equal(V({ type: "object", minProperties: 5 }, {}).ok, false, "minProperties 不该被忽略");
  assert.equal(V({ type: "object", maxProperties: 1 }, { a: 1, b: 2 }).ok, false, "maxProperties 不该被忽略");
});

// 审查 Finding 2：2020-12 里 $ref 是普通适用型，同级关键字仍生效（AND）。Draft-07 才抑制同级。
test("审查 Finding 2：$ref 的同级关键字仍生效（2020-12 语义，不是 Draft-07）", () => {
  const strS = { $defs: { S: { type: "string" } }, $ref: "#/$defs/S", maxLength: 1 };
  assert.equal(V(strS, "hi").ok, false, "$ref 指向 string，同级 maxLength:1 仍该拒绝 'hi'");
  const objS = { $defs: { O: { type: "object" } }, $ref: "#/$defs/O", required: ["x"] };
  assert.equal(V(objS, {}).ok, false, "$ref 指向 object，同级 required 仍该报缺 x");
  const bothS = { $defs: { S: { type: "string" } }, $ref: "#/$defs/S", type: "number" };
  assert.equal(V(bothS, "hi").ok, false, "$ref 目标说 string、同级 type 说 number → 'hi' 违反 number");
  // $ref 同级的**未支持断言**也不该被漏掉（Finding 1 + 2 的交叉）
  const unsup = { $defs: { S: { type: "string" } }, $ref: "#/$defs/S", pattern: "^x" };
  assert.ok(V(unsup, "hi").errors.some((e) => e.keyword === "pattern"), "$ref 同级的未支持断言也要提示");
});
