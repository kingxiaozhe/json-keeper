...
- [x] T-010: 大文件降级说明 + "构建树"入口 + 建树加载态 ~30min
  - 涉及模块: `status.js`、`core.js`、`viewer.css`
  - **双层 rAF**（`rAF(() => rAF(build))`）让骨架先真的绘制出来 —— 单层 rAF 是错的（回调在本帧 paint **之前**跑，骨架一帧都不会出现），详见 design.md「建树加载态的正确做法」。主线程仍会阻塞，本版不假装解决（分片建树在 feature 5）。
  - 顺手修了一个 **v0.8.0 就在线上**的 bug：`if (heavy) setView("raw")` 把系统的决定写进了跨会话偏好 → 打开一个大 JSON，此后每个小 JSON 都默认 Raw。连带把 `applySort` / `ensurePretty` 两个调用点一起改成 `persist=false`（审查发现：只改 heavy 那行的话，**用过 ⇅ Sort 的人会经由异步回调完整拿回这个 bug**）。
  - 不覆盖：**搜索触发的建树不给骨架**（`search.run` 走同步 `renderTree`）→ 见 design.md，随 T-506 分片建树一并解决。
  - 未覆盖：状态栏窄窗口裁切（Node 量不出宽度）→ 冒烟第 29 条；骨架"真的被画出来"→ 冒烟第 27 条。
...
- [x] T-012: 全量手动冒烟清单 + 复跑 T-001 防护网基线 ~30min
